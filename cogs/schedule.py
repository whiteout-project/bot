...
class MinisterChannelView(discord.ui.View):
    ...
    @discord.ui.button(label="Day 1 Construction", style=discord.ButtonStyle.primary, emoji="👷")
    async def day1(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.guild.id != log_guild.id:
            await interaction.response.send_message(
                ...
                "If you want to change the server, run `/settings` command --> Other feature --> Minister Channels --> Delete server ID and run the `/minister_add` command in the desired server",
                ephemeral=True
            )
            return

        await interaction.response.send_message(content="Select a channel for Day 1 Construction:", view=ChannelSelectView(self.bot, "Day 1 Construction channel"), ephemeral=True)

    @discord.ui.button(label="Day 2 Research", style=discord.ButtonStyle.primary, emoji="🧑‍⚕️")
    async def day2(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(content="Select a channel for Day 2 Research:", view=ChannelSelectView(self.bot, "Day 2 Research channel"), ephemeral=True)

    @discord.ui.button(label="Day 4 Training", style=discord.ButtonStyle.primary, emoji="💂")
    async def day4(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(content="Select a channel for Day 4 Training:", view=ChannelSelectView(self.bot, "Day 4 Training channel"), ephemeral=True)
...
class Schedule(commands.Cog):
    ...
    async def appointment_autocomplete(self, interaction: discord.Interaction, current: str):
        try:
            choices = [
                discord.app_commands.Choice(name="Day 1 Construction", value="Day 1 Construction"),
                discord.app_commands.Choice(name="Day 2 Research", value="Day 2 Research"),
                discord.app_commands.Choice(name="Day 4 Training", value="Day 4 Training")
            ]
            if current:
    ...
    async def time_autocomplete(self, interaction: discord.Interaction, current: str):
        try:
            ...
            # Generate valid 30-minute interval times
            valid_times = {
                f"{hour:02}:{minute:02}"
            ...
            available_times = valid_times - booked_times
            ...
            if current:
                ...
                # Ensure input is in valid 30-minute intervals
    ...
    async def choice_autocomplete(self, interaction: discord.Interaction, current: str):
        try:
            choices = [
                ...
                discord.app_commands.Choice(name="Show available time only", value="available only")
            ]
    ...
    async def minister_add(self, interaction: discord.Interaction, appointment_type: str, fid: str, time: str):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"{interaction.user.mention} added someone to {appointment_type}.",
            ...
            message_content = f"**{appointment_type}** open holes:\n" + "\n".join(
                time_list) if available_slots else f"All holes are filled for {appointment_type}"
    ...
    async def minister_remove(self, interaction: discord.Interaction, appointment_type: str, fid: str):
        ...
        try:
            ...
            # Send embed confirmation to log channel
            embed = discord.Embed(
                title=f"{interaction.user.mention} removed someone from {appointment_type}.",
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"Are you sure you want to remove all minister appointment slots for: {appointment_type}?\n"
                            ...
                            f"You have 5 seconds to reply with 'Yes' to confirm or 'No' to cancel.",
            ...
            try:
                response = await self.bot.wait_for('message', check=check, timeout=5.0)
                ...
                if response.content.lower() == "yes":
                    ...
                    message_content = f"**{appointment_type}** list:\n" + "\n".join(time_list)
                    ...
                    embed = discord.Embed(
                        title=f"{interaction.user.mention} cleared `{appointment_type}` list",
        ...
        try:
            ...
            if all_or_available == "all":
                if update:
                    async def update_progress(checked, total, waiting):
                        if checked % 1 == 0:
                            ...
                            embed = discord.Embed(
                                title=title,
                                description=f"Checked {checked}/{total} hole fillers",
                                color=color
                            )
                            try:
                ...
                if available_slots:
                    ...
                    await interaction.followup.send(f"{appointment_type} open holes:\n{time_list}")
                else:
                    await interaction.followup.send(f"All holes are filled for {appointment_type}")
