...
class PaginationView(discord.ui.View):
    ...
    @discord.ui.button(emoji=f"⬅️", style=discord.ButtonStyle.blurple, disabled=True)
    async def previous_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._handle_page_change(interaction, -1)

    @discord.ui.button(emoji=f"➡️", style=discord.ButtonStyle.blurple)
...
class AllianceMemberOperations(commands.Cog):
    ...
    async def handle_member_operations(self, interaction: discord.Interaction):
        embed = discord.Embed(
            title=f"{pimp.allianceIcon} Alliance Member Operations",
            description=(
        ...
        class MemberOperationsView(discord.ui.View):
                    ...
                    async def select_callback(interaction: discord.Interaction):
                        ...
                        member_embed = discord.Embed(
                            ...
                            description=(
                                ...
                                f"{pimp.divider1}\n"
                                f"\n"
                                f"{pimp.allianceIcon} Total Member     : {len(members)}\n"
                        ...
                        async def member_callback(member_interaction: discord.Interaction, selected_fids=None, delete_all=False):
                            ...
                            if delete_all or (selected_fids and len(selected_fids) == len(members)):
                                ...
                                confirm_embed = discord.Embed(
                                    title=f"⚠️ Confirmation Required",
            ...
            @discord.ui.button(label="Transfer Members", emoji=f"{pimp.transferIcon}", style=discord.ButtonStyle.secondary, row=0)
                        ...
                        try:
                            ...
                            member_embed = discord.Embed(
                                ...
                                description=(
                                    ...
                                    f"**Selection Methods**\n"
                                    f"1️⃣ Select member from menu below\n"
                                    f"2️⃣ Click 'Select by ID' button and enter ID\n"
                                    f"{pimp.divider2}\n"
                                ),
                                color=pimp.emColor1
                            )
                            ...
                            async def member_callback(member_interaction: discord.Interaction, selected_fids=None):
                                ...
                                target_embed = discord.Embed(
                                    title=f"🎯 Target Alliance Selection",
                                    description=(
    ...
    async def add_user(self, interaction: discord.Interaction, alliance_id: str, ids: str):
        ...
        if queue_position > 1:  # Not the first in queue
            ...
            embed = discord.Embed(
                title=f"{pimp.allianceIcon} User Addition Progress", 
        ...
        try:
            ...
            try:
                with sqlite3.connect('db/settings.sqlite') as settings_db:
                    ...
                    if alliance_log_result and alliance_log_result[0]:
                        log_embed = discord.Embed(
                            title=f"{pimp.allianceIcon} Members Added to Alliance",
                            description=(
                                f"**Alliance:** {alliance_name}\n"
...
class AllianceSelectView(discord.ui.View):
        ...
        for alliance_data in current_alliances:
            ...
            options.append(
                discord.SelectOption(
                    ...
                    emoji=f"{pimp.verifiedIcon}" if len(alliance_data) == 4 and alliance_data[3] else "🏰"
                )
            )
    ...
    @discord.ui.button(label=f"◀️", style=discord.ButtonStyle.secondary)
                ...
                if self.context == "remove":
                    embed = discord.Embed(
                        ...
                        description=(
                            ...
                            f"⚠️ **Are you sure you want to delete this member?**"
                        ),
                        color=emColor2
                    )
...
class AllianceSelectViewWithAll(discord.ui.View):
    def __init__(self, alliances_with_counts, cog):
        ...
        select = discord.ui.Select(
            placeholder=f"🏰 Select an alliance or ALL...",
            options=options
        )
    ...
    async def toggle_alliance_button(self, interaction: discord.Interaction):
        if self.include_alliance:
            ...
            if not any(self.selected_columns.values()):
                ...
                self.update_buttons()
                await interaction.response.edit_message(
                    content=f"⚠️ At least one column must be selected!",
                    view=self
                )
            else:
                await interaction.response.edit_message(view=self)
...
class MemberSelectView(discord.ui.View):
    ...
    def update_select_menu(self):
        ...
        if self.is_remove_operation:
            placeholder_text = f"{pimp.allianceIcon} Select members to remove (Page {self.page + 1}/{self.max_page + 1})"
        else:
    ...
    async def update_main_embed(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"{pimp.divider1}\n"
                f"\n"
                f"{pimp.allianceIcon} Total Members: {len(self.members)}\n"
                f"{pimp.stoveIcon}️ Highest Level: {self.cog.level_mapping.get(max_fl, str(max_fl))}\n"
                f"📈 Average Level: {self.cog.level_mapping.get(int(avg_fl), str(int(avg_fl)))}\n"
                f"\n"
                f"{pimp.divider1}\n"
    ...
    @discord.ui.button(label=f"◀️", style=discord.ButtonStyle.secondary, row=1)
